...
async function scrape() {
  ...
  for (const siteSchema of siteScrapingSchemas) {
    ...
    const selectors = Object.fromEntries(
      Object.entries(siteSchema.selectors).filter(
        ([field, value]) =>
          typeof value === "string" && field !== "block" && field !== "parser"
      )
    );
    await page.waitForSelector(siteSchema.selectors.block);
    const data = await page.$$eval(
      siteSchema.selectors.block,
      async (blocks: Element[], selectors, url) => {
        const parser = {
        ...
        return blocks.map((block, index) =>
          Object.entries(selectors).reduce(
            (scrapeData: OpenBook, [field, selector]) => {
